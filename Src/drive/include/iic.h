...
typedef struct {
    /* 单位 ns */
    unsigned short t_hold_start; /* 总线开始时间 */
    unsigned short t_scl_low;    /* SCL低电平时长 */
    unsigned short t_scl_high;   /* SCL高电平时长 */
    unsigned short t_setup_start;/* 开始设置时间 */
    unsigned short t_hold_sda;   /* 数据保持时长 */
    unsigned short t_setup_sda;  /* 数据设置时长 */
    unsigned short t_raise;      /* SCL, SDA上升沿时间 */
    unsigned short t_fall;       /* SCL, SDA下降沿时间 */
    unsigned short t_setup_stop; /* 停止设置时间 */
    unsigned short t_bus_free;   /* 总线空余时间 */
    unsigned short t_valid_data; /* 数据有效期 */
    unsigned short t_vaild_ack;  /* 数据确认时间 */
} i2c_speed_t;

...
static IIC_GPIOInterfaceTypeDef _gt9147_pin;
static IIC_DeviceTypeDef _gt9147_device;
...
unsigned char GT9147_Send_Cfg(IIC_DeviceTypeDef *dev, unsigned char mode)
{
	...
	GT9147_WR_Reg(dev, GT_CHECK_REG, buf, 2); /* 写入校验和,和配置更新标记 */
	return 0;
}
                            ...
                            unsigned char *buf, unsigned char len)
{
	...
	unsigned char ret = 0;
        IIC_Start(dev);	
        IIC_WriteByte(dev, dev->IIC_Device_Address); /* IIC Write */ 
        IIC_WaitAck(dev);
                   ...
                   unsigned char *buf, unsigned char len)
{
	unsigned char i;
        ...
        IIC_WaitAck(dev);
        IIC_Start(dev);
        IIC_WriteByte(dev, dev->IIC_Device_Address + 1);
        IIC_WaitAck(dev);
        for (i = 0; i < len; i++)
...
unsigned char GT9147_Init(void)
{
	unsigned char temp[5];
        ...
        IIC_DeviceTypeDef *dev = &_gt9147_device;

        _gt9147_pin.SCL.Pin  = GPIO_PIN_6;
...
const unsigned int GT9147_TPX_TBL[5] = {GT_TP1_REG, GT_TP2_REG, GT_TP3_REG,
...
{
 	static unsigned char t = 1; /* 控制查询间隔,从而降低CPU占用率 */
	unsigned char i;
        unsigned char mode;
	unsigned char temp;
	...
	if ((t % 10) == 0 || t < 10) {
		GT9147_RD_Reg(dev, GT_GSTID_REG, &mode, 1); /* 读取触摸点的状态 */
 		if ((mode & 0X80) && ((mode & 0XF) < 6)) {
			temp = 0;
			GT9147_WR_Reg(dev, GT_GSTID_REG, &temp, 1);  /* 清标志 */
		}		
		if ((mode & 0X0F) && ((mode & 0XF) < 6)) {
			temp = 0XFF << (mode & 0XF); /* 将点的个数转换为1的位数,匹配__lcd_tp_device.sta定义 */
			tempsta = __lcd_tp_device.sta; /* 保存当前的__lcd_tp_device.sta值 */
			__lcd_tp_device.sta = (~temp) | TP_PRES_DOWN | TP_CATH_PRES; 
			__lcd_tp_device.x[4] = __lcd_tp_device.x[0]; /* 保存触点0的数据 */
			__lcd_tp_device.y[4] = __lcd_tp_device.y[0];
			for (i = 0; i < CT_MAX_TOUCH; i++) {
				if (__lcd_tp_device.sta & (1 << i)) { /* 触摸有效? */
			...
			if (__lcd_tp_device.x[0] > __ltdc_lcd_device.active_heigh ||
				...
				if ((mode & 0XF) > 1) {
					...
					t = 0; /* 触发一次,则会最少连续监测10次,从而提高命中率 */
                                        ...
                                        mode = 0X80;
                                        __lcd_tp_device.sta = tempsta; /* 恢复__lcd_tp_device.sta */
				}
			}
                        else /* 触发一次,则会最少连续监测10次,从而提高命中率 */
                                t = 0;
		}
	}
        /* 无触摸点按下 */
